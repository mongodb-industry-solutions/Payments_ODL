...
const AccountsTable = () => {
  ...
  const handleSubmit = async () => {
    if (!accountBalance || !accountType) {
      alert('Account type and balance is required');
      return;
    }
    const login = JSON.parse(localStorage.getItem('login'));
    const bud = {
  ...
  return (
    <div className="table-wrapper">
        ...
        {!isFormOpen && (
            <Button size={'default'} onClick={openForm} style={{marginTop: '10px',}} >
              {'Open a new account'}
            </Button>)}
            {isFormOpen && (
              <Card style={{width:'250px'}} >
                <div className="form" >
                  ...
                  <div>
                    <Button size={'default'} onClick={closeForm} style={{marginTop: '10px',marginRight:'10px'}} > Close Form </Button>
