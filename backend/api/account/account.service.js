...
async function preWarmConnection(){
    try {
        await Promise.all([dbService.getEncryptedCollection('accounts', serviceName, encryptedFieldsMap, atlasSearchIndex)])
...
async function search(txt) {
    ...
    try {
        ...
        const { collection } = await dbService.getEncryptedCollection('accounts', serviceName, encryptedFieldsMap, atlasSearchIndex);
        let searchStage = { }
        ...
        if (txt) {
            ...
            matchStage = { $match: { "user.username" : { $regex: `^${txt}`, $options: 'i' } } }
